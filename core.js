(function(root, factory) {
  ...
  // ✅ УСИЛЕННЫЙ BUILTIN: добавлены документы с "пересекающимися" признаками
  const BUILTIN = [
    ...
    {t:'ДОГОВОР АРЕНДЫ №12.\nАрендодатель передаёт помещение 50 кв.м. Арендатор обязуется вносить ежемесячную плату 45 000 руб. до 5 числа на р/с 40702... Договор действует 1 год. Не является счётом на оплату.', y:'договор', p:'средний', r:'Бухгалтерия', s:'нейтральный'}, // NEW: есть сумма и реквизиты, но явно договор
    ...
    {t:'КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ №11.\nООО «Софт» предлагает: лицензия 1С - 5 шт. по 85 000 руб. Действует 30 дней. Счёт будет выставлен после согласования. Не является договором или счётом.', y:'договор', p:'низкий', r:'Продажи', s:'нейтральный'}, // NEW: цены, сумма, но явно КП
    {t:'АКТ СВЕРКИ ВЗАИМОРАСЧЁТОВ за Q3.\nПо договору №44. Задолженность ООО «Вектор» составляет 320 000 руб. Просим подтвердить или указать расхождения. Не требует оплаты.', y:'акт', p:'средний', r:'Бухгалтерия', s:'нейтральный'}, // NEW: сумма, договор, но акт сверки
    {t:'СЧЁТ НА ОПЛАТУ №99.\nВ рамках договора №12 от 10.01.2026. Услуги хостинга, 12 мес. Сумма 144 000 руб., НДС 0%. Просим оплатить до 20.09. Подпись директора.', y:'счёт', p:'высокий', r:'Бухгалтерия', s:'нейтральный'}, // NEW: ссылка на договор, но счёт
    {t:'УНИВЕРСАЛЬНЫЙ ПЕРЕДАТОЧНЫЙ ДОКУМЕНТ №205.\nОтгрузочная накладная и акт приёмки в одном лице. Товар: кабель 500 м. Стоимость 450 000 руб. Отгружено со склада №2. Принято без замечаний.', y:'накладная', p:'средний', r:'Склад', s:'нейтральный'} // NEW: УПД, классифицируем как накладная
  ];
      ...
      for(let j=0;j<GSIZE;j++){
        if(j < NF*NH){
          ...
          if(li < LEX.length) {
            ...
            // Структурные признаки: чуть выше база для лучшей чувствительности к формату
            g.push((Math.random() - 0.5) * 0.8);
          }
        } else {
